...
spp_trait <- matrix(runif(n_spp, -1,1), nrow = n_spp)
...
for(i_param in 1:nrow(all_betas)){
  ...
  for(i in 1:n_rep){
    ...
    spp_mat <- simple_sortingIBT(spp_trait, island_feature2, this_beta)
    perm_sample <- rho_permutation(spp_mat, trait_dist)
    res[i_save,1] <- perm_sample$p$lower
    res[i_save,2:4] <- this_beta[-1]
    res[i_save,5] <- i
    i_save <- i_save + 1
  }
  write.csv(res, "./tests/formal_simulations/logistic_IBT/p_vals_less_traits2.csv",row.names = F)
}
